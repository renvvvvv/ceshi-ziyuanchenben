...
if ! ssh -o BatchMode=yes -o ConnectTimeout=5 -o StrictHostKeyChecking=no "$SSH_TARGET" "echo ok" &>/dev/null; then
...
TARBALL="/tmp/deploy-platform-$$.tar.gz"
...
# 解压新代码（保留 pg_data 卷）
tar xzf /tmp/deploy-platform-$$.tar.gz
rm -f /tmp/deploy-platform-$$.tar.gz
