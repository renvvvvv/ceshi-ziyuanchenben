...
router.post('/', requireAuth, async (req, res) => {
  ...
  try {
    ...
    const systemPrompt = personaPrompt +
      ...
      '你的知识库内置了三大厂（阿里巴巴/腾讯/字节跳动）的数据中心测试验收标准，它们与国标 GB50174-2017、GB50462-2024 的对应关系，以及 26 本国标/行标规范全文（含数据中心设计/施工验收、低压配电、防雷、暖通施工、消防报警/联动/气体灭火、智能建筑、防静电、机柜、活动地板等，检索来源文件名以 std- 开头）。回答涉及国标条款时优先引用全文库原文，给出【标准号+条款号】。\n' +
      '回答要求：\n' +
...
router.post('/learn', requireAuth, (req, res) => {
  ...
  // 防注入/防配额炸：question/answer 都会全量进所有用户后续问答的 system prompt，双双限长
  if (answer.trim().length > 2000) {
    return res.status(400).json({ success: false, message: '补充内容过长（≤2000 字），请精简后提交' });
  }
  if (question.trim().length > 500) {
    return res.status(400).json({ success: false, message: '问题内容过长（≤500 字）' });
  }
