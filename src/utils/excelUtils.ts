...
export function exportReportToExcel(input: ResourceInput, report: ResourceReport): void {
  ...
  // Sheet 2: 人员投入清单
  {
    const data = [
      ...
      [9, '消防主测', 1, dur, `消防测试员${r.消防.测试员}人（${input.total_cabinets}柜）`],
      [10, '消防测试员', r.消防.测试员, r.消防.测试员 * dur, `${input.total_cabinets}柜，每850柜增1人`],
      [11, '柴发主测', 1, dur, ''],
      [12, '柴发记录员', 1, dur, ''],
      [13, '资料员', 1, dur, '文档管理/数据整理'],
  ...
  // Sheet 5: 机柜PDU + 劳务
  {
    const data = [
      ...
      ['PDU', r.机柜PDU清单.PDU.count, r.机柜PDU清单.PDU.days, r.机柜PDU清单.PDU.totalUnits],
      ['', '', '', ''],
      ['劳务总人天', r.劳务清单.总人天, '', ''],
      ['参考项目', `${r.项目信息.总容量} ${r.项目信息.空调} 项目，工期${r.项目信息.工期}`, '', ''],
    ];
    const { ws } = aoaToSheet(data, [16, 10, 8, 10]);
  ...
  // Sheet 6: 详细计算数据
  {
    const detailData: (string | number)[][] = [
      ...
      ['暖通记录员', r.弱电.暖通记录员], ['记录员小计', r.弱电.记录员小计],
      ['弱电小计', r.弱电.小计],
      ['', ''],
      ['=== 消防 / 柴发 / 固定人员 ===', ''],
